...
(function($) {
    $(document).ready(function () {
            ...
            if (countries.length) {
                countries.on('change', function () {
                    ...
                    if ( $.inArray(which_fields, [ 'country_state', 'all' ] ) !== -1 ) {
                        const d = acfps_get_states(country_code, show_labels, post_id);
                        ...
                        const d = acfps_get_places(country_code, show_labels, post_id);
                        response_places.push(d);

                        Promise.all(response_places).then(function(jsonResults) {
                            for (i = 0; i < jsonResults.length; i++) {
            ...
            if (state.length) {
                state.on('change', function () {
                    ...
                    if ( 'all' === which_fields || which_fields.indexOf("city") >= 0 ) {
                        ...
                        var changed_city = $('select[id="' + city_field_id + '"]');
                        const d = acfps_get_places(state_code, show_labels, post_id);
                        response_places.push(d);
         ...
         */
        function acfps_get_states(countryCode, showLabels, postID, callback) {
            const state_data = {
         ...
         * @returns {Promise<unknown>}
         */
        function acfps_get_places(stateCode, showLabels, postID, callback) {
            const city_data = {
                action: 'get_places_call',
                post_id: postID,
                show_labels: showLabels,
