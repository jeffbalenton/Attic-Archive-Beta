<?php
     ...
     * @return array
     */
    function acfps_get_field_settings( $fields = array() ) {
                    ...
                    if ( empty( $fields ) ) {
                        ...
                        foreach( $groups as $key => $values ) {
                            ...
                            foreach( $group_field as $field ) {
                                if ( in_array( $field[ 'type' ], [ 'ACF_Place_Selector', 'repeater', 'flexible_content', 'group' ] ) ) {
                                    $new_fields[] = $field;
                                }
                            }
                            if ( isset( $new_fields ) ) {
            ...
            if ( isset( $fields ) && is_array( $fields ) && count( $fields ) > 0 ) {
                foreach( $fields as $field ) {
                    if ( isset( $field[ 'type' ] ) && $field[ 'type' ] == 'ACF_Place_Selector' ) {
                        ...
                        $array_key = array_search( 'ACF_Place_Selector', array_column( $field[ 'sub_fields' ], 'type' ) );
                        if ( false !== $array_key ) {
                        ...
                        foreach( $layouts as $layout ) {
                            ...
                            $array_key  = array_search( 'ACF_Place_Selector', array_column( $sub_fields, 'type' ) );
                            if ( false !== $array_key ) {
                                ...
                                if ( false !== $repeater_key ) {
                                    $array_key = array_search( 'ACF_Place_Selector', array_column( $sub_fields[ $repeater_key ][ 'sub_fields' ], 'type' ) );
                                    if ( false !== $array_key ) {
                            ...
                            if ( false === $array_key ) {
                                ...
                                if ( false === $array_key ) {
                                    ...
                                    if ( ! empty( $field[ 'sub_fields' ][ $array_key ][ 'sub_fields' ] ) ) {
                                        $acf_key = array_search( 'ACF_Place_Selector', array_column( $field[ 'sub_fields' ][ $array_key ][ 'sub_fields' ], 'type' ) );
                                        if ( false !== $acf_key ) {
