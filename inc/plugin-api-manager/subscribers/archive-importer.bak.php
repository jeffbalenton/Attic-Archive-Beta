...
$GLOBALS['max_wxr_version'] = 1.2; // max. supported WXR version
$GLOBALS['id']; // WXR attachment ID
...
$GLOBALS['processed_authors']= [];
$GLOBALS['author_mapping']= [];
$GLOBALS['processed_terms']= [];
$GLOBALS['processed_posts']= [];
$GLOBALS['processed_menu_items']= [];
$GLOBALS['post_orphans']= [];

$GLOBALS['menu_item_orphans'];
$GLOBALS['missing_menu_items'];


$GLOBALS['fetch_attachments'] = false;
$GLOBALS['url_remap'] = [];
$GLOBALS['featured_images'] = [];
/**
 * The main controller for the actual import stage.
 *
 * @param string $file Path to the WXR file for importing
 */
function load_starter_content() {

  $import_check = get_option( 'import-check' );
  if ( $import_check != 'done' ):


  add_filter( 'import_post_meta_key', 'is_valid_meta_key' );
...
function import_end() {
  ...
  exit;
  /*
        echo '<p>' . __( 'All done.', 'wordpress-this' ) . ' <a href="' . admin_url() . '">' . __( 'Have fun!', 'wordpress-this' ) . '</a>' . '</p>';
        echo '<p>' . __( 'Remember to update the passwords and roles of imported users.', 'wordpress-this' ) . '</p>';
  */
}
...
function get_author_mapping() {
  ...
  foreach ( ( array )$authors as $author ) {
    ...
    if ( !isset( $author_mapping[ $santized_old_login ] ) ) {
      if ( $old_id )
        $processed_authors[ $old_id ] = ( int )get_user_by( 'display_name', 'Webmaster' );
      $author_mapping[ $santized_old_login ] = ( int )get_user_by( 'display_name', 'Webmaster' );
    }
  }
}
...
function process_categories() {
  ...
  foreach ( $categories as $cat ) {
    ...
    if ( !is_wp_error( $id ) && $id > 0 ) {
      ...
      printf( __( 'Failed to import category %s', 'wordpress-this' ), esc_html( $cat[ 'category_nicename' ] ) );
  ...
  foreach ( $tags as $tag ) {
    ...
    if ( $term_id ) {
      ...
      if ( isset( $tag[ 'term_id' ] ) )
        $processed_terms[ intval( $tag[ 'term_id' ] ) ] = ( int )$term_id;
      continue;
    }
    ...
    if ( !is_wp_error( $id ) ) {
      if ( isset( $tag[ 'term_id' ] ) )
        $processed_terms[ intval( $tag[ 'term_id' ] ) ] = $id[ 'term_id' ];
    } else {
    ...
    process_termmeta( $tag, $id[ 'term_id' ] );
  }
...
function process_terms() {
  ...
  foreach ( $terms as $term ) {
    ...
    process_termmeta( $term, $id[ 'term_id' ] );
  }
 ...
 */
function process_termmeta( $term, $term_id ) {
...
function process_posts() {
  ...
  foreach ( $posts as $post ) {
    ...
    if ( isset( $processed_posts[ $post[ 'post_id' ] ] ) && !empty( $post[ 'post_id' ] ) )
      continue;
    ...
    if ( $post_exists && get_post_type( $post_exists ) == $post[ 'post_type' ] ) {
      ...
      $processed_posts[ intval( $post[ 'post_id' ] ) ] = intval( $post_exists );
    } else {
    ...
    $processed_posts[ intval( $post[ 'post_id' ] ) ] = ( int )$post_id;
...
function process_menu_item( $item ) {
  ...
  if ( !$menu_slug ) {
    _e( 'Menu item skipped due to missing menu slug', 'wordpress-this' );
    echo '<br />';
    return;
  }
  ...
  if ( !$menu_id ) {
    printf( __( 'Menu item skipped due to invalid menu slug: %s', 'wordpress-this' ), esc_html( $menu_slug ) );
    echo '<br />';
    return;
  } else {
  ...
  if ( $id && !is_wp_error( $id ) )
    $processed_menu_items[ intval( $item[ 'post_id' ] ) ] = ( int )$id;
}
 ...
 */
function get_filename_from_disposition( $disposition_header ) {
  // Get the filename.
  $filename = null;
