...
implements SubscriberInterface {
  ...
  public static function get_subscribed_hooks() {
    return [
      ...
      'acf/save_post' => 'normalized_birth_date',
      //'save_post' => [ 'normalized_death_date', '20', '2' ],
      'acf/save_post' => 'convert_event_date',
  ...
  function convert_event_date() {
    ...
    $event_month = get_field( 'event_month', $post_id );
    $event_day = get_field( 'event_day', $post_id );
    $event_year = get_field( 'event_year', $post_id );


    $event_date = date_create( "$event_year-$event_month-$event_day" );
    $formatted_event_date = date_format( $event_date, "M j, Y" );

    if ( $formatted_event_date ) {
      update_post_meta( $post_id, 'event_date', $formatted_event_date );
    }
  ...
  function normalized_birth_date( $post_id ) {
    ...
    // $formal_bdate = $bmonth . ' ' . $bday . ', ' . $byear;
  ...
  function normalized_death_date( $post_id ) {
    // Get newly saved values.
    $values = get_fields( $post_id );
    ...
    $formal_ddate = "$dmonth $dday $dyear";

    if ( $formal_ddate ) {
      update_post_meta( $post_id, 'deathdate', $dmonth . ' ' . $dday . ', ' . $dyear );
    }
  ...
  public function load_starter_content() {
    ...
    $this->create_content();
    $this->import_end();
    update_option( 'import-check', 'yessir' );
    endif;
    /*    
