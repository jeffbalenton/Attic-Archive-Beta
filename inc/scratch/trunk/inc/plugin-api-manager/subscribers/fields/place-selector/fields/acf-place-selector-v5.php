    ...
    if ( ! class_exists( 'acf_field_city_selector' ) ) {
        ...
        class acf_field_city_selector extends acf_field {
            ...
            function __construct( $settings ) {
                ...
                $this->name     = 'ACF_Place_Selector';
                $this->label    = 'City Selector';
                $this->category = esc_attr__( 'Choice', 'acf-place-selector' );
                $this->defaults = array(
                    'show_labels'  => 1,
                    'which_fields' => 'all',
                    'use_select2'  => 0,
                ...
                $no_countries               = esc_attr__( 'No countries', 'acf-place-selector' );
                $select_country             = esc_attr( apply_filters( 'acfps_select_country_label', __( 'Select a country', 'acf-place-selector' ) ) );
                $select_city                = esc_attr( apply_filters( 'acfps_select_city_label', __( 'Select a city', 'acf-place-selector' ) ) );
                $select_country_first       = esc_attr( apply_filters( 'acfps_select_country_first', __( 'No results (yet), first select a country', 'acf-place-selector' ) ) );
                $select_state               = esc_attr( apply_filters( 'acfps_select_province_state_label', __( 'Select a province/state', 'acf-place-selector' ) ) );
                $select_state_first         = esc_attr( apply_filters( 'acfps_select_state_first', __( 'No results (yet), first select a state', 'acf-place-selector' ) ) );
            ...
            function render_field_settings( $field ) {
                ...
                $label_options = array(
                    1 => esc_attr__( 'Yes', 'acf-place-selector' ),
                    0 => esc_attr__( 'No', 'acf-place-selector' )
                );
                acf_render_field_setting( $field, array(
                    'choices'      => $label_options,
                    'instructions' => esc_html__( 'Show field labels above the dropdown menus', 'acf-place-selector' ),
                    'label'        => esc_html__( 'Show labels', 'acf-place-selector' ),
                    'layout'       => 'horizontal',
                    'name'         => 'show_labels',
                    'type'         => 'radio',
                ...
                acf_render_field_setting( $field, array(
                    'choices'      => $label_options,
                    'instructions' => esc_html__( 'Use select2 for dropdowns', 'acf-place-selector' ),
                    'label'        => esc_html__( 'Select2', 'acf-place-selector' ),
                    'layout'       => 'horizontal',
                    'name'         => 'use_select2',
                    'type'         => 'radio',
                ...
                $countries = acfps_get_countries( true, false, true );
                acf_render_field_setting( $field, array(
                    'choices'      => $countries,
                    'instructions' => esc_html__( 'Select a default country for a new field', 'acf-place-selector' ),
                    'label'        => esc_html__( 'Default country', 'acf-place-selector' ),
                    'name'         => 'default_country',
                    'type'         => 'select',
                ) );

                $default_country_fields = array(
                    'all'           => esc_attr__( 'All fields [default]', 'acf-place-selector' ),
                    'country_only'  => esc_attr__( 'Country only', 'acf-place-selector' ),
                    'country_state' => esc_attr__( 'Country + State/province', 'acf-place-selector' ),
                    'country_city'  => esc_attr__( 'Country + City', 'acf-place-selector' ),
                    'state_city'    => esc_attr__( 'State/province + City', 'acf-place-selector' ),
                );
                acf_render_field_setting( $field, array(
                    'choices'      => $default_country_fields,
                    'instructions' => esc_html__( 'Select which fields are used', 'acf-place-selector' ),
                    'label'        => esc_html__( 'Fields to use', 'acf-place-selector' ),
                    'name'         => 'which_fields',
                    'type'         => 'radio',
                ) );
            }
            ...
            function render_field( $field ) {
                ...
                if ( false !== $default_country && false == $selected_country ) {
                    ...
                    if ( in_array( $which_fields, [ 'all', 'country_state', 'state_city' ] ) ) {
                        $prefill_states = acfps_get_states( $default_country, $show_first, $field );
                    }
                    if ( in_array( $which_fields, [ 'country_city', 'state_city' ] ) ) {
                        $prefill_places = acfps_get_places( $default_country, false, $field );
                    }

                } elseif ( false !== $selected_country ) {
                    if ( in_array( $which_fields, [ 'all', 'country_state', 'state_city' ] ) ) {
                        $prefill_states = acfps_get_states( $selected_country, $show_first, $field );
                    }
                    if ( in_array( $which_fields, [ 'all', 'country_city', 'state_city' ] ) ) {
                        $prefill_places = acfps_get_places( $selected_country, $selected_state, $field );
                    }
                    ...
                    echo '<div class="acfps"><div class="acfps__notice field__message field__message--error">';
                    esc_html_e( "You haven't set a default country, so NO provinces/states and places will be loaded.", 'acf-place-selector' );
                    echo '</div></div>';
                }

                $prefill_values = [
                    'prefill_states' => $prefill_states,
                    'prefill_places' => $prefill_places,
                ];

                if ( 'state_city' != $which_fields ) {
                    echo acfps_render_dropdown( 'country', $field, $selected_country, $prefill_values );
                }
                if ( 'all' == $which_fields || strpos( $which_fields, 'state' ) !== false ) {
                    echo acfps_render_dropdown( 'state', $field, $selected_state, $prefill_values );
                }
                if ( 'all' == $which_fields || strpos( $which_fields, 'city' ) !== false ) {
                    echo acfps_render_dropdown( 'city', $field, $selected_city, $prefill_values );
                }
            }
            ...
            function input_admin_enqueue_scripts() {
                ...
                wp_register_script( 'acfps-init', "{$plugin_url}assets/js/init.js", array( 'jquery', 'acf-input' ), $plugin_version );
                wp_enqueue_script( 'acfps-init' );

                wp_register_script( 'acfps-process', "{$plugin_url}assets/js/place-selector.js", array( 'jquery', 'acf-input' ), $plugin_version );
                wp_enqueue_script( 'acfps-process' );

                // check field settings
                $all_info                     = acfps_get_field_settings();
                ...
                $js_vars[ 'show_labels' ]     = ( isset( $all_info[ 'show_labels' ] ) ) ? $all_info[ 'show_labels' ] : apply_filters( 'acfps_show_labels', true );
                ...
                wp_localize_script( 'acfps-process', 'city_selector_vars', $js_vars );
            ...
            function load_value( $value, $post_id, $field ) {
                ...
                if ( strlen( $country_code ) == 2 && false != $state_code ) {
                    global $wpdb;
                    $sql_query              = $wpdb->prepare( "SELECT country, state_name FROM {$wpdb->prefix}places WHERE country_code= %s AND state_code= %s", $country_code, $state_code );
            ...
            function validate_value( $valid, $value, $field, $input ) {
                ...
                if ( 1 == $field[ 'required' ] ) {
                    $nothing       = esc_html__( "You didn't select anything.", 'acf-place-selector' );
                    $no_city       = esc_html__( "You didn't select a city.", 'acf-place-selector' );
                    $no_country    = esc_html__( "You didn't select a country.", 'acf-place-selector' );
                    $no_state      = esc_html__( "You didn't select a state.", 'acf-place-selector' );
                    $no_state_city = esc_html__( "You didn't select a state and city.", 'acf-place-selector' );
