<?php
     ...
     */
    function acfps_city_selector_ajaxurl() {
        ?>
    ...
    add_action( 'wp_head', 'acfps_city_selector_ajaxurl' );
    add_action( 'login_head', 'acfps_city_selector_ajaxurl' );
     ...
     * @return JSON Object
     */
    function acfps_get_states_call() {
        ...
        if ( isset( $_POST[ 'country_code' ] ) ) {
            ...
            if ( false != $post_id ) {
                ...
                if ( is_array( $fields ) && ! empty( $fields ) ) {
                    $field = acfps_get_field_settings( $fields );
                }
            }
            ...
            if ( isset( $country_code ) ) {
                $states_transient = acfps_get_states( $country_code, true, $field );
            }
    ...
    add_action( 'wp_ajax_get_states_call', 'acfps_get_states_call' );
    add_action( 'wp_ajax_nopriv_get_states_call', 'acfps_get_states_call' );


    /*
     * Get places by state code and/or country code
     *
     * @return JSON Object
     */
    function acfps_get_places_call() {
        ...
        if ( isset( $_POST[ 'state_code' ] ) ) {
            ...
            $places_transient = acfps_get_places( $country_code, $state_code, $field );

            if ( ! empty( $places_transient ) ) {
                foreach ( $places_transient as $city ) {
                    $items[] = [
                        'id'        => $city,
                        'city_name' => $city,
                    ];
    ...
    add_action( 'wp_ajax_get_places_call', 'acfps_get_places_call' );
    add_action( 'wp_ajax_nopriv_get_places_call', 'acfps_get_places_call' );
