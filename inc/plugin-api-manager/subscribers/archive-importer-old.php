...
if ( !class_exists( 'Starter_Content' ) ):
	  ...
	  implements SubscriberInterface
 {
  ...
  function import_start( $file ) {
   ...
   if ( !is_file( $file ) ) {
      echo '<p><strong>' . __( 'Sorry, there has been an error.', 'wordpress-this' ) . '</strong><br />';
      echo __( 'The file does not exist, please try again.', 'wordpress-this' ) . '</p>';
    ...
    function get_authors_from_import( $import_data ) {
      if ( !empty( $import_data[ 'authors' ] ) ) {
        ...
        foreach ( $import_data[ 'posts' ] as $post ) {
          ...
          if ( empty( $login ) ) {
            printf( __( 'Failed to import author %s. Their posts will be attributed to the current user.', 'wordpress-this' ), esc_html( $post[ 'post_author' ] ) );
            echo '<br />';
            continue;
          }
  ...
  function get_author_mapping() {
    ...
    foreach ( ( array )$this->authors as $author ) {
      ...
      if ( !isset( $this->author_mapping[ $santized_old_login ] ) ) {
        if ( $old_id )
          $this->processed_authors[ $old_id ] = ( int )get_user_by( 'display_name', 'Webmaster' );
        $this->author_mapping[ $santized_old_login ] = ( int )get_user_by( 'display_name', 'Webmaster' );
      }
    }
  }
  ...
  function process_tags() {
    ...
    foreach ( $this->tags as $tag ) {
      ...
      if ( !is_wp_error( $id ) ) {
        ...
        printf( __( 'Failed to import post tag %s', 'wordpress-this' ), esc_html( $tag[ 'tag_name' ] ) );
        if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG )
          echo ': ' . $id->get_error_message();
        echo '<br />';
        continue;
  ...
  function process_terms() {
    ...
    foreach ( $this->terms as $term ) {
      ...
      if ( !is_wp_error( $id ) ) {
        ...
        printf( __( 'Failed to import %s %s', 'wordpress-this' ), esc_html( $term[ 'term_taxonomy' ] ), esc_html( $term[ 'term_name' ] ) );
        if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG )
          echo ': ' . $id->get_error_message();
        echo '<br />';
        continue;
  ...
  function process_posts() {
    ...
    foreach ( $this->posts as $post ) {
      ...
      if ( !post_type_exists( $post[ 'post_type' ] ) ) {
        printf( __( 'Failed to import &#8220;%s&#8221;: Invalid post type %s', 'wordpress-this' ),
      ...
      if ( $post_exists && get_post_type( $post_exists ) == $post[ 'post_type' ] ) {
        printf( __( '%s &#8220;%s&#8221; already exists.', 'wordpress-this' ), $post_type_object->labels->singular_name, esc_html( $post[ 'post_title' ] ) );
        echo '<br />';
        ...
        if ( is_wp_error( $post_id ) ) {
          printf( __( 'Failed to import %s &#8220;%s&#8221;', 'wordpress-this' ),
        ...
        foreach ( $post[ 'terms' ] as $term ) {
          ...
          if ( !$term_id ) {
            ...
            if ( !is_wp_error( $t ) ) {
              ...
              printf( __( 'Failed to import %s %s', 'wordpress-this' ), esc_html( $taxonomy ), esc_html( $term[ 'name' ] ) );
              if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG )
